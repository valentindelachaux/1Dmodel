...
import pandas as pd
import numpy as np
import back_heat_transfer as bht
...
def h_top(parameters,var):
    ...
    h = parameters["coeff_h_top"]*bht.front_h_simple(T_PV,T_amb,parameters["theta"],parameters["longueur"])
...
def h_inner(parameters,var):
    if var["T_abs_mean"]==None:
        ...
        if parameters["geometry"]=="harp":
            D_4 = parameters["D_4"]
            parameters["h_inner"] = parameters["coeff_h_back"]*bht.back_h_fins(var["T_abs_mean"],parameters["T_back"],parameters["theta"],parameters["longueur"],D_4,parameters["lambd_ail"],parameters["Heta"])

        else:
            D = (parameters["largeur"]-parameters["N_ail"]*parameters["lambd_ail"])/(parameters["N_ail"]-1)
            parameters["h_inner"] = parameters["coeff_h_back"]*bht.back_h_fins(var["T_abs_mean"],parameters["T_back"],parameters["theta"],parameters["longueur"],D,parameters["lambd_ail"],parameters["Heta"])
    else:
...
def h_inner_mean(parameters,var):
    if parameters["geometry"] == "harp":
        ...
        new_h_inner = parameters["coeff_h_back"]*bht.back_h_fins(var["T_abs_mean"],parameters["T_back"],parameters["theta"],parameters["longueur"],D_4,parameters["lambd_ail"],parameters["L_a"])
        ...
        if parameters["N_ail"]<= 24:
            ...
            new_h_inner = parameters["coeff_h_back"]*bht.back_h_fins(var["T_abs_mean"],parameters["T_back"],parameters["theta"],parameters["longueur"],D,parameters["lambd_ail"],parameters["Heta"])
        if abs(new_h_inner - old_h_inner) > 0.1:
            parameters["h_inner"] = (old_h_inner+new_h_inner)/2
        else:
            pass
...
def KTE(parameters, var):
    ...
    if parameters["fin_3"]==1:
        ...
        D1 = (D_tube*a*N)/L_riser
        D2 = D_tube*(L_riser-N*a)/L_riser
        #D2=0
        alpha = (2*Bi)**(1/2)


        delta_vf = parameters["delta_f3"]
        A = (-k_ail*alpha*N*math.cosh((alpha*L_vf)/a))/(L_riser*math.sinh((alpha*L_vf)/a))
        Aet = (delta_vf*N*a*math.cosh((alpha*L_vf)/a))/(L_riser)

        chiet = chi-(1/A)
        #chiet = chi

        K = Aet*(-D1*Fprime*(h_rad+1/R_T)+(-D2*Fprime)*(1/(R_B*Fprime)))-2*k_abs*lambd_abs*m*tanh_or_inverse(m*L_af)
        T = Aet*(1+chiet*(D1*Fprime*(h_rad+1/R_T)+(D2*Fprime)*(1/(R_B*Fprime)))+2*k_abs*lambd_abs*m*tanh_or_inverse(m*L_af)*chiet)
        E = Aet*(D1*Fprime*(S+h_rad*T_sky+T_amb/R_T)+(D2*Fprime)*(T_back/(R_B*Fprime))+2*k_abs*lambd_abs*m*tanh_or_inverse(m*L_af)*(b/j))
...
def one_loop(parameters,T_fluid_in,var):
    ...
    if parameters["fin_3"] == 1:
        Bi_f3(parameters) # directement dans le calcul de KTE()
